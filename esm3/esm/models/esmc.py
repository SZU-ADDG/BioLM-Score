...
class ESMC(nn.Module, ESMCInferenceClient):
    ...
    def from_pretrained(
        ...
        # Check if device is provided, if not, set it to 'cuda' or 'cpu' based on availability
        if device is None:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        local_weights_path = "/data1/chenby/codes/esm-main/trained_model/esmc_600m_2024_12_v0.pth"

        client = load_local_model("esmc_600", device=device)
