...
class ESMProtein(ProteinType):
    ...
    def to_protein_complex(
        self, copy_annotations_from_ground_truth = None
    ) -> ProteinComplex:
        assert (
...
class ESMProteinTensor(ProteinType):
    ...
    def to(self, device_or_dtype: Union[str, torch.device, torch.dtype]) -> ESMProteinTensor:
        def _to(name):
            v = getattr(self, name)
...
class LogitsOutput:
    ...
    residue_annotation_logits = None
    hidden_states = None
