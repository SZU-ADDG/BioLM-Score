...
def _default_data_path(x, d: PathLike) -> PathLike:
    return x if x is not None else C.data_root("esm3") / d


def _default_local_data_path(x, d: PathLike) -> PathLike:
...
class InterProQuantizedTokenizer(EsmTokenizerBase):
    ...
    def __init__(
        ...
        lsh_path= None,
        keyword_vocabulary_path = None,
        keyword_idf_path = None,
        interpro_entry_path = None,
        interpro2keywords_path = None,
    ):
    ...
    def get_special_tokens_mask(self, encoded: torch.Tensor) -> torch.Tensor:
        ...
        assert torch.all(torch.all(where, dim=1) or torch.all(~where, dim=1))
        return where[:, 0]
    ...
    def _function_text_hash(
        self, labels: Collection[str], keyword_mask = None
    ) -> Optional[np.ndarray]:
    ...
    def lookup_annotation_name(self, annotation: FunctionAnnotation) -> Optional[str]:
