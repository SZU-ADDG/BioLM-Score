...
from typing import Any, ContextManager, Sequence, TypeVar, Union, Optional
...
def unbinpack(
    tensor: torch.Tensor, sequence_id :Optional[torch.Tensor], pad_value: Union[int,float]
):
    """
    Args:
        tensor (Tensor): [B, L, ...]
...
# def fp32_autocast_context(device_type: str) -> ContextManager:
#     """
#     No-op context manager to enforce full float32 (no AMP).
#     """
#     yield


def merge_ranges(ranges: list[range], merge_gap_max = None) -> list[range]:
...
def merge_annotations(
    annotations: list[FunctionAnnotation], merge_gap_max = None
...
def maybe_tensor(x, convert_none_to_nan: bool = False) -> Optional[torch.Tensor]:
    if x is None:
        return None
...
def maybe_list(x, convert_nan_to_none: bool = False) -> Optional[list]:
    if x is None:
        return None
