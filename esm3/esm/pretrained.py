...
def ESM3_structure_encoder_v0(device):
...
def ESM3_structure_decoder_v0(device):
    with torch.device(device):
...
def ESM3_function_decoder_v0(device):
    with torch.device(device):
...
def ESMC_300M_202412(device, use_flash_attn: bool = True):
    with torch.device(device):
        model = ESMC(
            d_model=960,
            n_heads=15,
...
def ESMC_600M_202412(device, use_flash_attn: bool = True):
    with torch.device(device):
        model = ESMC(
            d_model=1152,
            n_heads=18,
