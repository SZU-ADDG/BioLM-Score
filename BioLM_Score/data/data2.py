...
class VSDataset(Dataset):
	def __init__(self,  
		...
		if isinstance(ligand_emb,str):
			...
			self.ligand_emb = th.tensor(self.ligand_emb,dtype=th.float)
...
class VSDataset2(Dataset):
    def __init__(self,
        ...
        super(VSDataset2, self).__init__()
        self.labels = labels
        self.gp = None
        self.gls = None
        self.pocketdir = None
        ...
        if isinstance(ligs, np.ndarray) or isinstance(ligs, list):
            if isinstance(ligs[0], Chem.rdchem.Mol):
                ...
                raise ValueError('Ligands should be a list of rdkit.Chem.rdchem.Mol objects')
        elif isinstance(ligs, Chem.rdchem.Mol):
            self.ligs = [ligs]
            self.gls = [self._mol_to_graph0(ligs)]
        elif isinstance(ligs, Data):
            self.gls = [ligs]
        else:
            if ligs.endswith(".mol2"):
                lig_blocks = self._mol2_split(ligs)
