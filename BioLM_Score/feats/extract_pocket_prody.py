...
#import subprocess
os.environ["BABEL_LIBDIR"] = "/data1/×××/anaconda3/bin/obabel"
...
def extract_pocket(protpath, 
	...
	# ret = xcom.select(f'same residue as exwithin %s of resname %s and not resname {lresname}' % (cutoff, lresname))
	ret = xcom.select(f'same chain as exwithin %s of resname %s and not resname {lresname} and not hetatm' % (cutoff, lresname))
	...
	print("%s/%s_pocket_chain_%sA.pdb"%(workdir, protname, cutoff))
