...
#     center_pos = th.tensor(np.array([
#         res.atoms.center_of_mass() if (np.all(np.isfinite(res.atoms.masses)) and np.sum(res.atoms.masses) > 0)
#         else res.atoms.center_of_geometry()
...
#     if th.isnan(g.x).any() or th.isnan(g.edge_attr).any():
#         print(f"[NaN Detected] Protein graph from {prot} has NaN in node features!")
...
#     res1 = u.residues[i]
#     res2 = u.residues[j]
#     if((res1.segid.strip() == res2.segid.strip()) and (abs(res1.resid - res2.resid) == 1)):
#         return 1
#     else:
#         return 0
...
def load_mol(molpath, explicit_H=False, use_chirality=True):
    ...
    if re.search(r"\.pdb$", molpath, flags=re.IGNORECASE):
        mol = None
        ...
        # 2) 若读取失败（包括返回 None 或抛异常），走 fallback
        if mol is None:
            mol = Chem.MolFromPDBFile(molpath, sanitize=False, removeHs=False)
            if mol is None:
                raise ValueError("RDKit failed to read PDB even with sanitize=False")

            # 忽略/清空电荷
            for a in mol.GetAtoms():
                a.SetFormalCharge(0)

            # 可选：尝试 sanitize，但不让它中断
            try:
...
# def protein_remove_h(prot_path):
#     full_u = mda.Universe(prot_path)
#     protein_no_h = full_u.select_atoms("not name H*")
#     new_pdb_path = os.path.splitext(prot_path)[0] + "_no_h.pdb"
#     protein_no_h.write(new_pdb_path)
#     return new_pdb_path
...
def label_query(pdbid, df):
    return df.loc[pdbid, "affinity"]

def pdbbind_handle(pdbid, i,  args):
    prot_path = "%s/%s/%s_pocket_chain_%sA.pdb"%(args.dir, pdbid, pdbid, args.cutoff)
    lig_path = "%s/%s/%s_ligand_sort.sdf"%(args.dir, pdbid, pdbid)
...
def UserInput():
    ...
    p.add_argument('-c', '--cutoff', default=None, type=int,
                        help='the cutoff to determine the pocket')
    p.add_argument('-o', '--outprefix', default="out",
                        help='The output bin file.')
    p.add_argument('-r', '--ref', default="/data1/chenby/codes/GenScore-main/data/train_data/pdbbind_affinity.csv",
...
def main():
    args = UserInput()
    # casf_ids_path = '/data1/***/codes/GenScore-main/data/train_data/v2020_casf_ids.csv'
    # casf_ids = pd.read_csv(casf_ids_path, header=None, dtype=str)[0].tolist()
    casf_pdbids = np.load('/data1/***/codes/GenScore-main/data/rtmscore_s/v2020_casf_ids.npy')[0]

    pdbids = [x for x in os.listdir(args.dir)
              if os.path.isdir("%s/%s"%(args.dir, x)) and x in casf_pdbids]
    ...
    if args.parallel:
        ...
        for i, pdbid in enumerate(pdbids):
            # 序列特征和图特征无法对应的情况
            if pdbid in ['1pum','4ucs', '2ylc', '4fxf', '4ucv','4ucr','4uvb', '4uco', '1utt', '4aaw']:
                continue
            results.append(pdbbind_handle(pdbid,i,  args))
